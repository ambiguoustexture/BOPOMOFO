...
def chinese_character_2_bopomofo(chinesecharacters):
    """
    将汉字转换为注音符号表示
    :param chinesecharacters: 待转换的汉字字符串
    :return:                  对应的注音符号
    """
    return re.sub(r'[\[\]\'\', ]', '', str(pinyin(chinesecharacters, style=Style.BOPOMOFO)))
...
if __name__ == "__main__":
    ...
    for line in open("./pinyin.txt"):
        index = line.find("=")
        bopomofo = chinese_character_2_bopomofo(line[ : index])
